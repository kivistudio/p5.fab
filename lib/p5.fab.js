...
class Fab {
  ...
  print() {
    ...
    if (this.commandStream.length > 0) {
      ...
      console.log('print');
      console.log(this.commandStream[0]+ "\n");
  ...
  introLine(z = 0.3) {
    if (this.coordinateSystem != "delta") {
      ...
      this.moveExtrude(0.1, 200, z, 25, 1);
      this.addComment("intro");
      this.move(0.4, 200.0, z, 85);
      this.moveExtrude(0.4, 20.0, z, 25, 1);
      this.addComment("intro");
    } else {
      for (let angle = 0; angle <= TWO_PI / 3; angle += TWO_PI / 50) {
        ...
        if (angle == 0) {
          ...
          fab.moveExtrude(this.centerX + x, this.centerY + y, z, 5, 1);
        }
      }
    }
  ...
  moveExtrude(x, y, z, v, e = 1) {
    let fCmd = "";
    let eCmd = "";
    if(v) {
      this.v = parseFloat(this.mm_sec_to_mm_min(v)).toFixed(2);
      fCmd = ` F${this.v}`;
    } 
    if(e) {
      console.log(e, this.makeE(x, y, z));
    ...
    this.y = parseFloat(y).toFixed(2);
    this.z = parseFloat(z).toFixed(2);
    let cmd = `G1 X${this.x} Y${this.y} Z${this.z} ${fCmd} ${eCmd}`;
    this.add(cmd);
  ...
  setFeedrate(v) {
    let feedRate = this.mm_sec_to_mm_min(v);
    this.v = parseFloat(feedRate).toFixed(2);
    var cmd = `G1 F${this.v}`;
    this.addFirst(cmd);
  }
