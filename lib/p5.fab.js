...
class Fab {
  ...
  print() {
    ...
    if (this.commandStream.length > 0) {
      ...
      let command = this.commandStream[0];
      let newCommand = this.replaceVariables(command);
      this.serial.write(newCommand + "\n");
  ...
  replaceVariables(command) {
    let defaultFReg = "FVAR\\((\\d*\.?\\d*)\\)";
    let defaultF = myp5.match(command, defaultFReg);
    if(!defaultF) {
      return command;
    }
    if(this.fOverride) {
      return command.replace(/FVAR\(\d*.?\d*\)/g, 'F' + this.fOverride);
    } else {
      return command.replace(/FVAR\(\d*.?\d*\)/g, 'F'+defaultF[1]);
    }
  }
    ...
    if (!this.model) {
      ...
      for (let v in this.vertices) {
        ...
        if (vertexData.command == "G0") {
          ...
          myp5.vertex(toolpathPos.x, toolpathPos.y, toolpathPos.z);
          myp5.vertex(vertexData.vertex.x, vertexData.vertex.y, vertexData.vertex.z);
          toolpathPos = toolpathPos.set([
            vertexData.vertex.x,
            vertexData.vertex.y,
            vertexData.vertex.z,
      ...
      myp5.endShape();
      this.model = myp5.saveShape();
    } else {
      myp5.model(this.model);
    }
    myp5.pop();
  ...
  drawDeltaPrinter() {
    ...
    // draw print bed
    myp5.translate(-this.radius, 0, -this.radius);
    myp5.rotateY(myp5.PI);
    myp5.scale(-1, 1);
    myp5.push();
    myp5.translate(this.radius, 0, this.radius);
    myp5.rotateY(myp5.PI / 12);
    myp5.rotateX(myp5.PI / 12);
    myp5.fill(254, 249, 152);
    myp5.push();
    myp5.translate(0, 2.5, 0);
    myp5.cylinder(this.radius + 1, 5); // build plate
    myp5.pop();

    myp5.push();
    myp5.noFill();
    myp5.translate(0, -this.maxZ / 2 + 1, 0);
    myp5.stroke(220, 50, 32);
    myp5.box((2 * this.radius) / myp5.sqrt(2), this.maxZ, (2 * this.radius) / myp5.sqrt(2)); // work envolope
    myp5.pop();
  ...
  configure(config) {
    ...
    if (config.coordinateSystem == "delta") {
      this.maxX = (2 * config.radius) / myp5.sqrt(2);
      this.maxY = this.maxX;
      this.centerX = 0;
      this.centerY = 0;
    } else {
  ...
  introLine(z = 0.3) {
    if (this.coordinateSystem != "delta") {
      ...
      for (let angle = 0; angle <= myp5.TWO_PI / 3; angle += myp5.TWO_PI / 50) {
        let x = 90 * myp5.cos(angle);
        let y = 90 * myp5.sin(angle);
        if (angle == 0) {
  ...
  up(z, v = 50) {
   ...
   // moveExtrude(x, y, z, v = 25, e = this.makeE(x, y, z)) {
    moveExtrudeNoOverride(x, y, z, v = 25, e = null, multiplier = false) {
  ...
  moveExtrude(x, y, z, v = 25, e = null, multiplier = false) {
    ...
    // this.e = e; // this assignment was removed
    var cmd = `G1 X${this.x} Y${this.y} Z${this.z} FVAR(${this.v}) E${this.e}`;
    this.add(cmd);

    return cmd;
  }

  // v - speed at which the head is moved
  ...
  overrideFeedrate(v) {
    console.log("overriding feedrate: ", v);
    this.fOverride = v;
    //var cmd = `G1 F${v}`;
    //this.addFirst(cmd);
  }

  overrideExtrudeRate(ratePercent) {
    var cmd = `M221 S${ratePercent}`;
    this.addFirst(cmd);
  }
  ...
  makeE(x, y, z) {
    return (
      myp5.dist(this.x, this.y, this.z, x, y, z) *
